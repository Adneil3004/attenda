    ...
    public async Task<EventDashboardDto> Handle(GetEventDashboardQuery request, CancellationToken cancellationToken)
    {
        try 
        {
            ...
            return new EventDashboardDto
            {
                ...
                TotalTables = @event.Tables.Count,
                TotalSeatingCapacity = @event.Tables.Sum(t => t.Capacity),
                SeatedGuestsCount = @event.Guests.Count(g => g.TableId != null),
                FullTablesCount = @event.Tables.Count(t => @event.Guests.Count(g => g.TableId == t.Id) >= t.Capacity),
                AvailableTablesCount = @event.Tables.Count(t => @event.Guests.Count(g => g.TableId == t.Id) < t.Capacity),
