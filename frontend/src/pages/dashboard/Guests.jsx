  ...
  const handleImportCSV = async (e) => {
    ...
    reader.onload = async (e) => {
      ...
      if (totalGuests + totalNewGuests > activeEvent.guest_limit) {
        alert(`Cannot import ${newGuests.length} guests (${totalNewGuests} total spaces). Your ${activeEvent.capacity_tier} plan allows up to ${activeEvent.guest_limit} spaces.`);
        return;
      }
          ...
          <div>
            ...
            {activeEvent && (
              <p className={`text-xs font-semibold mt-2 ${totalGuests > activeEvent.guest_limit ? 'text-red-600' : 'text-[var(--color-secondary)]'}`}>
                Event: {activeEvent.name} ({totalGuests} / {activeEvent.guest_limit} spaces used)
              </p>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            ...
            <div className="flex items-center gap-2 justify-end w-full sm:w-auto">
              ...
              <button 
                onClick={() => {
                  if (!hasRsvpConfig) {
                    setNotification({ type: 'warning', message: 'You must first create the RSVP design' });
                    return;
                  }
                }}
                ...
                title={hasRsvpConfig ? "Send registration link to all guests" : "Configure the RSVP in the Designer first"}
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" />
                </svg>
                <span className="hidden lg:inline">Send Invitations</span>
              </button>
          ...
          <div className="flex-1 overflow-auto">
            <div className={`primary-card overflow-hidden transition-all duration-300 ${isRefreshing ? 'ring-1 ring-[var(--color-secondary)]' : ''}`}>
              ...
              <div className="flex justify-between items-center px-6 py-4 border-b border-gray-50 bg-gray-50/30">
                <div className="flex items-center gap-3">
                  <span className="text-xs font-bold text-[var(--color-primary)] uppercase tracking-wider">Main List</span>
                  {isRefreshing && (
              ...
              <div className="grid grid-cols-12 gap-4 px-6 py-4 bg-[var(--color-surface-container-high)] border-b border-[var(--color-outline-variant)]/20 text-[11px] font-black uppercase tracking-widest text-[var(--color-primary)] items-center sticky top-0 z-10">
                ...
                <div className="col-span-6 md:col-span-4 lg:col-span-3">Guest</div>
                        ...
                        <button 
                          ...
                          onClick={async (e) => { 
                            ...
                            try {
                              ...
                              setNotification({ type: 'success', message: 'Invitation logged as sent!' });
                          ...
                          title={hasRsvpConfig ? "Send RSVP link (Logs action)" : "Configure RSVP first"}
                        >
