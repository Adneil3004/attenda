    ...
    <div className="flex flex-col gap-8 w-full p-2 animate-in fade-in duration-700">
      ...
      <div className="flex justify-between items-end mb-4">
        ...
        <button 
          ...
          className="bg-[var(--color-primary)] text-white px-6 py-3 rounded-xl font-bold text-xs uppercase tracking-widest shadow-lg hover:brightness-110 transition-all"
        >
          Nuevo Evento
        </button>
      </div>
      ...
      {/* ─── Events Grid ─── */}
      {events.length === 0 ? (
        <div className="bg-white rounded-3xl p-12 text-center border border-[var(--color-outline-variant)]/10">
          <p className="text-[var(--color-on-surface-variant)] font-semibold">No tienes eventos registrados aún.</p>
        </div>
      ) : (
          ...
          {events.map((event) => (
            ...
            >
              ...
              <div className="relative h-48 overflow-hidden bg-slate-100">
                <img 
                  src={event.imageUrl || 'https://images.unsplash.com/photo-1519224406070-51d3e2ef0dec?q=80&w=1000&auto=format&fit=crop'} 
                  alt={event.name} 
                ...
                {event.status === 'Active' && (
                  <div className="absolute top-4 left-4 bg-[#10b981] text-white text-[9px] font-extrabold px-3 py-1.5 rounded-full uppercase tracking-widest flex items-center gap-1.5 shadow-lg">
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h4 className="text-xl font-headline font-bold text-[var(--color-primary)] group-hover:text-[var(--color-secondary)] transition-colors line-clamp-1">
                      {event.name}
                    </h4>
                ...
                <div className="space-y-3 mb-8">
                  ...
                  <div className="flex items-center gap-3 text-[var(--color-on-surface-variant)]">
                    ...
                    <span className="text-xs font-bold text-[var(--color-primary)] line-clamp-1">{event.location || 'Sin ubicación'}</span>
                  </div>
                  <div className="flex items-center gap-3 text-[var(--color-on-surface-variant)]">
                    <div className="w-8 h-8 rounded-lg bg-[#f1f5f9] flex items-center justify-center">
                      <svg className="w-4 h-4 text-[var(--color-primary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <span className="text-xs font-bold text-[var(--color-primary)]">{event.guestCount} Invitados</span>
                  </div>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <button 
                    ...
                    className="w-12 h-11 bg-[#f1f5f9] text-[var(--color-primary)] rounded-xl flex items-center justify-center hover:bg-[#e2e8f0] transition-colors"
                  >
